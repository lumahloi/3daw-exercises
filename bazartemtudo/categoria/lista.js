$(function () {
  ...
  $("#deletar").click(() => {
    const id_categoria = $("#idCategoriaParaDeletar").val();
    $.ajax({
      type: "POST",
      url: "/BazarTemTudo/categoria/deletar.php",
      data: { id_categoria },
      success: (data) => {
        $("#mensagemModal").html("Categoria deletada com sucesso!");
        modal.show();
      },
      error: (xhr, status, error) => {
        $("#mensagemModal").html("Erro ao deletar categoria!");
        modal.show();
      },
    });
  });
...
function iniciarDeletar(id) {
  $("#idCategoriaParaDeletar").val(id);
  const modal = new bootstrap.Modal("#modalDelecao", {});
  modal.show();
}

function adicionarLinhaNaTabela(categoria) {
  $(
    "<tr><td id='categoria_descricao_" +
      ...
      categoria.id_categoria +
      "</td>" +
      "<td><button onclick='iniciarAlterar(" +
      categoria.id_categoria +
      "," +
      JSON.stringify(categoria.descricao) +
      ")'>Alterar</button>" +
      " " +
      "<button onclick='iniciarDeletar(" +
      categoria.id_categoria +
